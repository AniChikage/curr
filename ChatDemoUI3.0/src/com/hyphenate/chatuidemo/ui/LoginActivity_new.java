...
public class LoginActivity_new extends BaseActivity {
	...
	public void login(View view) {
		if(!user_role.isChecked()&&!consellor_role.isChecked()){
			...
			progressShow = true;
			final ProgressDialog pd = new ProgressDialog(LoginActivity_new.this);
			...
			EMClient.getInstance().login(currentUsername, currentPassword, new EMCallBack() {
				...
				public void onSuccess() {
					...
					if (!LoginActivity_new.this.isFinishing() && pd.isShowing()) {
						pd.dismiss();
					}
	...
	Handler cshandler=new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			if(string.indexOf("token")>0) {
				Intent intent =new Intent(LoginActivity_new.this, ConsellorPage.class);
				startActivity(intent);
				finish();
			}
			else{
				Toast.makeText(LoginActivity_new.this, "咨询师登录失败", Toast.LENGTH_SHORT).show();
			}
			Log.e("sdsdf",string);
			super.handleMessage(msg);
		}
